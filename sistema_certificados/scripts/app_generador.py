...
import qrcode
import pdfplumber
import subprocess  # Para abrir el explorador de archivos
...
class CertificadosApp(ctk.CTk):
    def __init__(self):
        ...
        # Ventana Principal
        self.title("Generador de Certificados IISEP v3.0")
    ...
    def setup_ui(self):
        ...
        # Título y subtítulo
        self.lbl_title = ctk.CTkLabel(self.frame_header, text="GENERADOR DE CERTIFICADOS", 
                                     font=("Roboto", 24, "bold"), text_color="white")
        self.lbl_title.grid(row=0, column=0, sticky="w", padx=30, pady=(15, 0))
        
        self.lbl_subtitle = ctk.CTkLabel(self.frame_header, text="Sistema de Estampado de Códigos QR", 
                                        font=("Roboto", 14), text_color="#b0b0b0")
        self.lbl_subtitle.grid(row=1, column=0, sticky="w", padx=30, pady=(0, 15))

        # Estado del sistema
        self.lbl_status_top = ctk.CTkLabel(self.frame_header, text="● Sistema listo", 
                                          text_color="#2ecc71", font=("Roboto", 12, "bold"))
        ...
        ctk.CTkLabel(self.frame_selection, text="SELECCIÓN DE ARCHIVOS", 
                    font=("Roboto", 14, "bold"), text_color="#e0e0e0").grid(row=0, column=0, sticky="w", padx=15, pady=(10, 5))
        ...
        self.btn_folder = ctk.CTkButton(self.btn_frame, text="📂 SELECCIONAR CARPETA", 
                                       command=self.select_folder, height=40, 
                                       fg_color="#34495e", hover_color="#2c3e50",
                                       font=("Roboto", 12, "bold"))
        self.btn_folder.grid(row=0, column=0, sticky="ew", padx=(0, 10))
        
        self.btn_file = ctk.CTkButton(self.btn_frame, text="📄 SELECCIONAR ARCHIVO", 
        ...
        self.lbl_path = ctk.CTkLabel(self.btn_frame, text="Ninguna ruta seleccionada", 
                                    text_color="#95a5a6", anchor="w", 
                                    font=("Roboto", 11))
        ...
        ctk.CTkLabel(self.frame_info, text="INFORMACIÓN DEL SISTEMA", 
                    font=("Roboto", 14, "bold"), text_color="#e0e0e0").grid(row=0, column=0, sticky="w", padx=15, pady=(10, 5))
        ...
        self.lbl_files = ctk.CTkLabel(self.stats_frame, text="Archivos listos: 0", 
                                     text_color="#3498db", font=("Roboto", 12))
        self.lbl_files.grid(row=0, column=0, sticky="w", padx=(0, 20))
        
        self.lbl_status = ctk.CTkLabel(self.stats_frame, text="Estado: Esperando", 
                                      text_color="#f39c12", font=("Roboto", 12))
        self.lbl_status.grid(row=0, column=1, sticky="w")

        # 3. ZONA DE REGISTRO (Consola mejorada)
        ...
        ctk.CTkLabel(console_header, text="REGISTRO DE ACTIVIDAD", 
                    font=("Roboto", 14, "bold"), text_color="#e0e0e0").grid(row=0, column=0, sticky="w", padx=15, pady=10)
        
        # Área de texto de la consola
        self.textbox = ctk.CTkTextbox(self.frame_console, font=("Consolas", 12), 
                                     fg_color="#1e1e1e", text_color="#ecf0f1")
        self.textbox.grid(row=1, column=0, sticky="nsew", padx=10, pady=10)
        self.textbox.insert("0.0", ">> Bienvenido al sistema de estampado de QRs.\n>> Seleccione una carpeta o archivo para comenzar.\n\n")
        self.textbox.configure(state="disabled")

        # 4. BARRA DE ACCIONES Y PROGRESO
        self.frame_actions = ctk.CTkFrame(self, height=120, fg_color="#2d2d2d")
        self.frame_actions.grid(row=3, column=0, sticky="ew", padx=0, pady=0)
        self.frame_actions.grid_columnconfigure(0, weight=1)
        self.frame_actions.grid_rowconfigure(0, weight=1)
        self.frame_actions.grid_propagate(False)

        # Panel interno de acciones
        ...
        self.inner_actions.grid_rowconfigure(0, weight=1)
        self.inner_actions.grid_rowconfigure(1, weight=1)

        # Barra de progreso
        self.progress = ctk.CTkProgressBar(self.inner_actions, height=15, 
                                          progress_color="#27ae60", fg_color="#34495e")
        self.progress.grid(row=0, column=0, columnspan=3, sticky="ew", pady=(0, 15))
        self.progress.set(0)
        
        # Etiqueta de porcentaje
        self.lbl_progress = ctk.CTkLabel(self.inner_actions, text="0%", 
                                        text_color="#7f8c8d", font=("Roboto", 12))
        self.lbl_progress.grid(row=0, column=2, sticky="e", pady=(0, 15))

        # Botones de control
        self.btn_start = ctk.CTkButton(self.inner_actions, text="▶ INICIAR PROCESO", 
                                      command=self.start_thread, 
                                      font=("Roboto", 14, "bold"), height=45, 
                                      fg_color="#27ae60", hover_color="#2ecc71")
        self.btn_start.grid(row=1, column=0, sticky="ew", padx=(0, 10))

        self.btn_stop = ctk.CTkButton(self.inner_actions, text="⏹ DETENER", 
        ...
        self.btn_open_res = ctk.CTkButton(self.inner_actions, text="↗ ABRIR CARPETA", 
    ...
    def log(self, msg, color="white"):
        ...
        # Simple logging without color tags inside textbox for compatibility
    ...
    def select_folder(self):
        ...
        if path:
            ...
            # Pre-calcular ruta de salida para habilitar botón si ya existe
    ...
    def open_output_folder(self):
        """Abre el explorador de Windows en la carpeta de resultados"""
    ...
    def worker(self):
        ...
        self.btn_open_res.configure(state="normal") # Habilitar botón de abrir carpeta
    ...
    def process_one_pdf(self, filepath, filename):
        ...
        can.drawImage(qr_temp, 50, 50, width=90, height=90) # Ajustar posición
        ...
        output = PdfWriter()
        output.add_page(existing_pdf.pages[0].merge_page(new_pdf.pages[0]))
