...
COLORS = {
    ...
    "panel":      ("#ffffff", "#1e293b"),
    "text":       ("#0f172a", "#f8fafc"),
    "text_sec":   ("#64748b", "#cbd5e1"),
    "input":      ("#e2e8f0", "#334155"),
    "border":     ("#cbd5e1", "#475569"),
    "accent":     ("#0f5bb5", "#3b82f6"),
    "accent_hover": ("#0a458a", "#2563eb"),
    "success":    ("#10b981", "#059669"),
    "danger":     ("#ef4444", "#dc2626"),
    "preview_bg": ("#cbd5e1", "#020617")
}
...
class SingleCertApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("Sistema de Certificación IISEP v2.5")
    ...
    def setup_header(self):
        ...
        try:
            if os.path.exists(RUTA_LOGO_PNG):
                ...
                # [CAMBIO 2] Agregado anchor="center" para centrar verticalmente el logo
                ctk.CTkLabel(inner, image=self.logo_image_ref, text="").pack(side="left", padx=(0, 20), anchor="center")
        except: pass

        txt_frame = ctk.CTkFrame(inner, fg_color="transparent")
        # [CAMBIO 2 OPCIONAL] Aseguramos que el marco de texto también esté centrado
        txt_frame.pack(side="left", fill="y", pady=5, anchor="center") 
        ...
        self.switch.pack(side="right", anchor="center")
        self.switch.select()
    ...
    def setup_left_panel(self):
        ...
        # [CAMBIO 1] --- SECCIÓN DOCUMENTO BASE (MOVIDO ARRIBA) ---
        ...
        self.btn_load = ctk.CTkButton(scroll, text="📂 CARGAR PDF (PLANTILLA)", height=45, 
        ...
        # [CAMBIO 1] --- SECCIÓN DATOS ESTUDIANTE (MOVIDO ABAJO) ---
        # Aumenté un poco el pady superior (25) para separarlo de la sección de arriba
        ctk.CTkLabel(scroll, text="DATOS DEL ESTUDIANTE", 
                    font=("Roboto", 16, "bold"), text_color=COLORS["accent"]).pack(anchor="w", pady=(25, 15))
        
        self.entry_nom = add_field("Nombre Completo", ctk.CTkEntry, 
                                 placeholder_text="EJ: MARIA PÉREZ (EN MAYÚSCULAS)",
                                 placeholder_text_color=COLORS["text_sec"])
        
        self.entry_dni = add_field("DNI", ctk.CTkEntry, 
                                 placeholder_text="EJ: 70451234 (SOLO NÚMEROS)",
                                 placeholder_text_color=COLORS["text_sec"])
    ...
    def setup_right_panel(self):
        ...
        # Inicializamos con el label de texto, asegurándonos de que exista
        self.lbl_preview = ctk.CTkLabel(self.frame_right, 
    ...
    def show_preview(self, path):
        # Usamos la ESTRATEGIA NUCLEAR que funcionó
        try:
            doc = fitz.open(path)
            ...
            self.lbl_preview = ctk.CTkLabel(self.frame_right, text="", image=self.preview_image_ref)
            self.lbl_preview.grid(row=0, column=0, sticky="nsew", padx=20, pady=20)
            
        except Exception as e:
            print(f"Error Preview: {e}")
            if self.lbl_preview: self.lbl_preview.destroy()
            self.lbl_preview = ctk.CTkLabel(self.frame_right, text=f"⚠️ Error: {str(e)}", text_color="red")
            self.lbl_preview.grid(row=0, column=0, sticky="nsew", padx=20, pady=20)
            self.preview_image_ref = None
    ...
    def reset_form(self):
        ...
        self.selected_file_path = None
        self.generated_file_path = None
        self.lbl_filename.configure(text="Seleccione el PDF de la constancia")
        self.preview_image_ref = None
        
        # DESTRUIR Y RECREAR (Versión Texto)
        if self.lbl_preview:
            self.lbl_preview.destroy()
        ...
        self.btn_process.configure(state="normal", text="GENERAR CERTIFICADO", fg_color=COLORS["accent"])
    ...
    def start_processing(self):
        ...
        self.btn_process.configure(state="disabled", text="Procesando...", fg_color=COLORS["border"])
    ...
    def on_success(self):
        self.btn_process.configure(text="¡Certificado Generado!", state="normal", fg_color=COLORS["success"])
    ...
    def show_info(self):
        messagebox.showinfo("IISEP Digital", "Sistema de Validación de Certificados v2.5\n\nDesarrollado para IISEP.")
